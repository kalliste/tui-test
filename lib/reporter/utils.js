...
export const loadShellVersions = async (shell) => {
    return Promise.all(shell.map(async (shell) => {
        ...
        try {
            switch (shell) {
                ...
                case Shell.Xonsh: {
                    const output = (await execAsync(`${target} -m xonsh -V`)).stdout.trim();
...
export function fitToWidth(line, width, prefix) {
