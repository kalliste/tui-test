...
export async function toHaveFgColor(locator, expected, options) {
    const cells = await locator.resolve(options?.timeout ?? getExpectTimeout());
    const [result, errorCell] = hasFgColor(cells ?? [], expected, this.isNot ?? false);
    ...
    return {
        ...
        message: () => {
            if (!pass && !this.isNot) {
                return (`expect(${chalk.red("received")}).toHaveFgColor(${chalk.green("expected")})` +
            ...
            if (pass && this.isNot) {
                return (`expect(${chalk.red("received")}).not.toHaveFgColor(${chalk.green("expected")})` +
...
function toMatchingColorMode(expected, cell) {
    if (cell == null)
        return "";
    const { termCell } = cell;
...
function hasFgColor(cells, color, isNot) {
