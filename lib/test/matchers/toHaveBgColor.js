...
export async function toHaveBgColor(locator, expected, options) {
    ...
    return {
        ...
        message: () => {
            if (!pass && !this.isNot) {
                return (`expect(${chalk.red("received")}).toHaveBgColor(${chalk.green("expected")})` +
            ...
            if (pass && this.isNot) {
                return (`expect(${chalk.red("received")}).not.toHaveBgColor(${chalk.green("expected")})` +
...
function hasBgColor(cells, color, isNot) {
